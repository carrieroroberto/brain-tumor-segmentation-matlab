...
La seconda configurazione più efficace risulta essere \textit{FLAIR}, con \textit{Dice} medio pari a \(0.795\), seguita da \textit{Fusion2} con \textit{Dice} medio pari a \(0.755\). La sequenza \textit{T2}, invece, presenta prestazioni inferiori con \textit{Dice} medio pari a \(0.520\), mentre la \textit{T1c} è la configurazione meno affidabile in singolo, con \textit{Dice} medio pari a \(0.195\).
...
Dalla Figura~\ref{fig:res_128} si osserva che la segmentazione ottenuta con \textit{Fusion3} presenta una sovrapposizione quasi completa con la \textit{ground truth}, seguendo con buona precisione la forma della lesione.
...
Inoltre, la \textit{seed map} costruita sulla fusione multimodale localizza il seme in una regione interna alla lesione. Questo rende più efficace il \textit{Region Growing}, che può espandersi in una zona coerente dal punto di vista dell’intensità e della continuità spaziale. Il \textit{Marker-Controlled Watershed} riesce, quindi, a rifinire il contorno senza alterazioni.

Anche la sola sequenza \textit{FLAIR} fornisce un buon risultato, con \textit{Dice} pari a \(0.879\), mentre la \textit{T2} raggiunge una prestazione intermedia (\(0.645\)). Al contrario, la \textit{T1c} risulta inadeguata, con \textit{Dice} pari a \(0.048\), poiché produce una segmentazione estesa lungo una fascia orizzontale dell’immagine. Anche \textit{Fusion2} risente di questo comportamento anomalo della \textit{T1c} e ottiene un \textit{Dice} molto basso (\(0.318\)).
